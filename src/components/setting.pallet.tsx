    ...
    return (
        <div className='rounded-xl flex p-[24px] bg-white shadow-md border flex-col gap-3'>
            ...
            <div className='flex flex-col gap-2'>
                ...
                {/* {
                    customers.length == 0 ? <Skeleton  /> : <select className=' border rounded-md focus:outline-none cursor-pointer px-3 py-2 shadow-md ' onChange={(e: any) => setCustomer(e.target.value)} value={customer}>
                        {
                ...
                {
                    customers.length == 0 ? <Skeleton /> : <Select  onChange={(e: any) => setCustomer(e)} value={customer}>
                        {
                            customers.map((oCus: PropsCustomer, iCus: number) => {
            ...
            <div className='flex flex-col gap-2'>
                ...
                <div className='flex justify-end my-2'>
                    <Input   placeholder='ค้นหา' onChange={(e: ChangeEvent<HTMLInputElement>) => setSearchPallet(e.target.value)} />
                </div>
                {
                    loadPallet == true ? <Skeleton /> : <div className='h-[400px] overflow-y-auto py-3 border rounded-md  px-3 bg-[#f5f5f5]'>
                                            ...
                                            <div className='col-span-2'>
                                                {
                                                    palletOfModel.map((oPL: PropsPallet, iP: number) => {
                                                        return <div className={`${oPL.ACTIVE != 'ACTIVE' && 'opacity-40'} flex gap-2 items-center cursor-pointer select-none opacity-80 hover:opacity-100 transition-all duration-150`} key={iP} onClick={() => handleUpdatePallet({ ...oPL, ACTIVE: (oPL.ACTIVE == 'INACTIVE' ? 'ACTIVE' : 'INACTIVE') })}>
                                                            {
                                                                oPL.ACTIVE == 'INACTIVE' ? <AiOutlineBorder className={` opacity-80 hover:opacity-100 transition-all duration-150`} fontSize='small' /> : <AiFillCheckCircle className='opacity-80 hover:opacity-100 transition-all duration-150' fontSize='small' />
                                                            }
